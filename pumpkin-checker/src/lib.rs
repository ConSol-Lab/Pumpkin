...
pub fn verify_proof<Source: BufRead>(
    mut _model: Model,
    mut _proof: ProofReader<Source, i32>,
) -> Result<(), CheckError> {
    // You should make use of the following functions:
    // - `inferences::verify_inference` - Which automatically calls your implemented checkers.
    // - `deductions::verify_deduction` - Which you should implement yourself.
    // - `verify_conclusion` - Which you should also implement yourself.
    //
    // We provide you with a structure for storing facts.
    let _facts: BTreeMap<ConstraintId, Fact> = BTreeMap::new();

    todo!()
}

#[allow(unused, reason = "Will be used in the assignment")]
fn verify_conclusion(_model: &Model, _conclusion: &drcp_format::Conclusion<Rc<str>, i32>) -> bool {
    todo!()
}
