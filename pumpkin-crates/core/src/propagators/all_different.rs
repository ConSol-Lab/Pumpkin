...
impl<Var: IntegerVariable + 'static> Propagator for AllDifferentPropagator<Var> {
    ...
    fn debug_propagate_from_scratch(
        ...
        let residual =
            Self::build_residual_graph(&initial_graph, no_vars, &matches, &mut is_matched);
        ...
        if is_matched.contains(&false) {
            ...
            // explanation for conflict includes all variables reachable from a variable not part of
            // the matching
            let mut fail_reason: PropositionalConjunction =
                PropositionalConjunction::new(Vec::new());
            ...
            fail_reason = Self::add_predicates_from_node(
                &rev_graph,
                &index,
                &mut fail_reason,
                &mut context,
...
impl<Var: IntegerVariable + 'static> AllDifferentPropagator<Var> {
    ...
    /// finds a maximal matching in a given bipartite graph, where the first set has no_vars
    /// elements returns the matching as a list l, l[i] being the index of the matched element
    /// for each of the no_vars elements
    fn find_match(graph: &Vec<Node>, no_vars: usize, is_matched: &mut Vec<bool>) -> Vec<i32> {
        ...
        flow_graph.push(source);
        flow_graph.push(sink);
    ...
    // given a graph and ids of variables, finds strongly connected components, then prunes edges
    // that are in different components and not part of the matching
    pub(crate) fn find_sccs_remove_values(
        graph: &Vec<Node>,
        ...
        // this prunes with order taken into account to make explanations more general; not fully
        // functioning yet Self::prune_ordered(&to_prune, &what_scc, context, var_set.len(),
        // &self_vars, &mut rev_graph, inference_code)
    }

    /// propagates the pruning of variable-value pairs in to_prune, taking into account the order
    /// such that explanations are more general
    fn prune_ordered(
        to_prune: &BTreeMap<usize, Vec<usize>>,
        what_scc: &Vec<usize>,
        mut context: PropagationContextMut,
        no_var: usize,
        self_vars: &Box<[Var]>,
        ...
        // needs to return PropagationStatusCP
        ...
        // make a graph using scc as the nodes and edges to prune as the edges, to then
        // topologically sort
        ...
        while let Some(current_scc) = queue.pop() {
            ...
            for xi in nodes_prune {
                // xi is the variable we are pruning for; to_prune[xi] is the list of what edges to
                // prune
                for value in to_prune.get(&xi).unwrap_or(&empty_list) {
                    ...
                    let mut explanation: PropositionalConjunction =
                        PropositionalConjunction::new(Vec::new());
                    println!("{:?}", graph);
                    println!("{:?}", xi);
                    explanation = Self::add_predicates_from_node(
                        graph,
                        value,
                        &mut explanation,
                        &mut context,
    ...
    fn prune_traversal(
        ...
        for (xi, vals) in to_prune.iter() {
            for value in vals.iter() {
                let mut explanation: PropositionalConjunction =
                    PropositionalConjunction::new(Vec::new());
                // explanation should contain variables reachable from the node value of the
                // variable-value being pruned
                explanation = Self::add_predicates_from_node(
                    graph,
                    value,
                    &mut explanation,
    ...
    fn add_predicates_from_node(
        graph: &Vec<Node>,
        initial_node: &usize,
        explanation: &mut PropositionalConjunction,
        ...
        let new_explanation = explanation
            .clone()
            .extend_and_remove_duplicates(new_predicates.into_iter());
        new_explanation
    }
            ...
            for &xi in &scc {
                ...
                if xi < graph.len() {
                    for &e in &graph[xi].neighbours {
                        if !scc.contains(&e) {
                            // xi is the value, e the variable; ensure e is not the sink, but a
                            // variable, and that xi is a value
                            if graph[e].id != graph.len() - 1 && graph[xi].id >= no_vars {
                                // store the variable-value pairs that need to be pruned to do so
                                // later
...
mod tests {
    ...
    fn test_explanation_easy() {
        ...
        solver.propagate(propagator).expect("non-empty domain");
        // let propagator = solver.new_propagator(AllDifferentPropagator::new([x,
        // y])).expect("non-empty domain"); solver.propagate(propagator).expect("non-empty
        // domain");
    ...
    fn test_2scc() {
        ...
        solver.propagate(propagator).expect("non-empty domain");
        // let propagator = solver.new_propagator(AllDifferentPropagator::new([a, b, c, d,
        // e])).expect("non-empty domain"); solver.propagate(propagator).expect("non-empty
        // domain");
    ...
    fn test_2scc_explanation() {
        ...
        let expected1 = PropositionalConjunction::new(
            vec![
                predicate!(c >= 3),
                predicate!(c <= 5),
                predicate!(d >= 3),
    ...
    fn test_3scc_explanation() {
        ...
        let expected2 =
            PropositionalConjunction::new(vec![predicate!(c >= 3), predicate!(c <= 3)].into());
    ...
    fn test_3scc_2_explanation() {
        ...
        solver.propagate(propagator).expect("non-empty domain");
        // let propagator = solver.new_propagator(AllDifferentPropagator::new([a, b, c, d, e,
        // f])).expect("non-empty domain");
        ...
        let expected2 = PropositionalConjunction::new(
            vec![
                predicate!(c >= 3),
                predicate!(c <= 4),
                predicate!(d >= 3),
    ...
    fn testy() {
        ...
        // let propagator = solver.new_propagator(AllDifferentPropagator::new([x, y,
        // z])).expect("non-empty domain");
    ...
    fn test_no_match() {
        ...
        let expected: PropositionalConjunction =
            conjunction!([x >= 1] & [x <= 2] & [y >= 1] & [y <= 2] & [z >= 1] & [z <= 2]).into();
        // let err = solver.new_propagator(AllDifferentPropagator::new([x, y,
        // z])).expect_err("string"); let expected: Inconsistency = conjunction!([x >= 1] &
        // [x <= 1] & [y >= 1] & [y <= 2] & [z >= 1] & [z <= 2]).into();
        println!("{:?}", expected);
        match err {
