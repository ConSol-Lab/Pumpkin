    ...
    pub fn extract_clausal_core(&mut self, brancher: &mut impl Brancher) -> CoreExtractionResult {
        ...
        self.assumptions
            ...
            .unwrap_or_else(|| {
                ...
                let mut resolver = ResolutionResolver::with_mode(
...
impl ConstraintSatisfactionSolver {
        ...
        loop {
            ...
            if self.state.no_conflict() {
                ...
                if self.get_decision_level() > self.assumptions.len()
                    && self.restart_strategy.borrow().should_restart()
                {
    ...
    fn resolve_conflict_with_nogood(&mut self, brancher: &mut impl Brancher) {
        ...
        let conflict_analysis_context = ConflictAnalysisContext {
            assignments: &mut self.assignments,
            counters: &mut self.solver_statistics,
            solver_state: &mut self.state,
            reason_store: &mut self.reason_store,
    ...
    pub fn add_clause(
        ...
        if self.internal_parameters.conflict_resolver == ConflictResolver::HypercubeLinear {
            let _ = self.add_propagator(HypercubeLinearPropagatorArgs {
                hypercube_linear: HypercubeLinear::clause(predicates.into_iter().collect()),
                constraint_tag,
                is_learned: false,
            })?;
