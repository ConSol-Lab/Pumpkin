...
use crate::cumulative::options::CumulativeMergeStrategy;
use crate::cumulative::time_table::OverIntervalTimeTableType;

#[derive(Debug, Clone, Copy)]
pub(crate) struct TimeTableMerger {
    num_calls_since_merge: u32,
    strategy: CumulativeMergeStrategy,
    merge_constant: Option<u32>,
    average_time_table_length: CumulativeMovingAverage<usize>,
}

impl TimeTableMerger {
    pub(crate) fn new(strategy: CumulativeMergeStrategy, merge_constant: Option<u32>) -> Self {
        pumpkin_assert_simple!(
            !matches!(strategy, CumulativeMergeStrategy::Constant) || merge_constant.is_some(),
            "The `Constant` merge strategy was used, but no merge constant was provided."
        );
        Self {
            num_calls_since_merge: 0,
            strategy,
            merge_constant,
            average_time_table_length: Default::default(),
        }
    }

    pub(crate) fn was_called(&mut self) {
        self.num_calls_since_merge += 1;
    }

    pub(crate) fn merge_if_necessary<Var: IntegerVariable + 'static>(
        &mut self,
        ...
        match self.strategy {
            CumulativeMergeStrategy::Constant => {
                let should_merge = self.num_calls_since_merge >= self.merge_constant.unwrap();

                if should_merge {
                    self.merge(time_table);
                    merged = true;
                }
            }
            CumulativeMergeStrategy::Average => {
                if time_table.len() >= self.average_time_table_length.value() as usize {
                    self.merge(time_table);
                    merged = true;
                }
                self.average_time_table_length.add_term(time_table.len());
            }
            CumulativeMergeStrategy::Never => {}
            CumulativeMergeStrategy::Always => {
                self.merge(time_table);
                merged = true;
            }
        }
    ...
    fn merge<Var: IntegerVariable + 'static>(
        &mut self,
        time_table: &mut OverIntervalTimeTableType<Var>,
    ) {
        self.num_calls_since_merge = 0;

        if time_table.is_empty() {
            return;
        }

        let time_table_len = time_table.len() - 1;
        Self::merge_range(time_table, 0, time_table_len)
    }

    pub(crate) fn merge_range<Var: IntegerVariable + 'static>(
        time_table: &mut Vec<ResourceProfile<Var>>,
        start_index: usize,
        end_index: usize,
    ) {
        ...
        // To avoid needless splicing, we keep track of the range at which insertions will take
        // place
        ...
        while current_index < end {
            ...
            while current_index < end
                && Self::are_mergeable(&time_table[current_index], &time_table[current_index + 1])
            {
            ...
            if current_index > first {
                ...
                if let Some(replacing_range) = insertion_range.take() {
                    // The end is decreased by the number of removed profiles (taking into account
                    // that `|replacing_profiles|` profiles were added)
                    end -= (replacing_range.end - replacing_range.start) - replacing_profiles.len();
                    // The current index should point to after the range of newly added profiles
                    // which means that it is updated to be the start of
                    // `replacing_range` + the number of newly added profiles
                    // (note that we know that the profile directly after the
        ...
        pumpkin_assert_extreme!(
            (0..time_table.len() - 1)
                .all(|index| { !Self::are_mergeable(&time_table[index], &time_table[index + 1]) }),
