...
pub(crate) fn calculate_order_strength(
    ...
    transitive_closure.edge_count() as f64 / ((number_of_tasks * (number_of_tasks + 1)) / 2) as f64
}
...
fn calculate_free_float_of_task(
    ...
    transitive_closure
        .edges(rev_map[task_index])
        .map(|edge_reference| {
            let original_edge_endpoint_index = rev_map
                .iter()
                .position(|current| *current == edge_reference.target())
                .unwrap();
            assert!(
                solver.lower_bound(&variables[original_edge_endpoint_index])
                    <= solver.lower_bound(&variables[task_index]),
                "Expected task {} (EST {}) to start after {} (EST {}) - Edges {:?}",
                original_edge_endpoint_index,
                solver.lower_bound(&variables[original_edge_endpoint_index]),
                task_index,
                solver.lower_bound(&variables[task_index]),
                transitive_closure
...
pub(crate) fn calculate_disjunction_ratio(
    ...
    assert_eq!(
        transitive_closure.node_indices().len(),
        number_of_tasks as usize
    );
    let number_of_resources = resource_requirements.len();
    let mut all_edges = (0..number_of_tasks as usize)
        .flat_map(|task_index| {
            transitive_closure
                .edges(task_index)
                .map(move |edge_reference| (task_index, edge_reference.target()))
        })
        .collect::<Vec<_>>();
    (0..number_of_tasks as usize).for_each(|current| {
        (0..number_of_tasks as usize).for_each(|other| {
            ...
            }) && !all_edges.contains(&(current, other))
                && !all_edges.contains(&(other, current))
            {
                all_edges.push((current, other));
            }
        })
    });

    all_edges.len() as f64 / ((number_of_tasks * (number_of_tasks + 1)) as f64 / 2_f64)
}
