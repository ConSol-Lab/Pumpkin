...
use super::variable_structs::VariableCumulativeParameters;
...
#[derive(Debug)]
pub(crate) struct VariableTimeTable<Var> {
    /// Stores whether the time-table is empty
    is_time_table_empty: bool,
    /// Stores the input parameters to the cumulative constraint
    parameters: VariableCumulativeParameters<Var>,
...
impl<Var: IntegerVariable + 'static> VariableTimeTable<Var> {
    pub(crate) fn new(
        arg_tasks: &[ArgTask<Var>],
        capacity: i32,
        cumulative_options: CumulativePropagatorOptions,
    ) -> VariableTimeTable<Var> {
        let (tasks, mapping) = create_tasks(arg_tasks);
        let parameters =
            VariableCumulativeParameters::new(tasks, capacity, cumulative_options, mapping);
...
impl<Var: IntegerVariable + 'static> Propagator for TimeTableOverIntervalPropagator<Var> {
    ...
    fn synchronise(&mut self, context: StatefulPropagationContext) {
