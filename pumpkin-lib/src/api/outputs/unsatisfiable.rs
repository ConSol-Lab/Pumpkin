...
impl<'solver, 'brancher, B: Brancher> UnsatisfiableUnderAssumptions<'solver, 'brancher, B> {
    ...
    ///
    /// A core is defined as a (sub)set of assumptions which, given a set of constraints, when
    /// set together whill lead to an unsatisfiable instance. For example, if we two variables
    /// `x, y, z ∈ {0, 1, 2}` and we have the constraint `all-different(x, y, z)` then the
    /// assumptions `[[x = 1], [y <= 1], [y != 0]]` would constitute an unsatisfiable core.
    ///
