// A heap class where the keys range from [0, ..., n-1] and the values are nonnegative floating
// points. The heap can be queried to return key with the maximum value, and certain keys can be
// (temporarily) removed/readded as necessary It allows increasing/decreasing the values of its
// entries

// the implementation could be more efficient, currently more comparisons are done than necessary
// when sifting, and possibly the recursion could be unrolled
...
pub struct KeyValueHeap {
    values: Vec<f64>, /* contains the values stored as a heap. The value of key i is in position
                       * map_key_to_position[i] */
    map_key_to_position: Vec<u32>, /* shows the location of the value of the key i in the
                                    * values_ array */
...
impl KeyValueHeap {
    ...
    // Return the key with maximum value from the heap, or None if the heap is empty. Note that this
    // does not delete the key (see 'pop_max') to get and delete.  O(1)
    ...
    // Removes the entry with key 'key' (temporarily) from the heap if the key is present, otherwise
    // does nothing.  its value remains recorded internally and is available upon calling
    // 'restore_key'.  the value can still be subjected to 'divide_values'.
    //  O(logn)
    ...
    pub fn grow(&mut self, value: f64) {
        ...
        self.map_key_to_position.push(new_key); // initially the key is placed placed at the very end, will be placed in the correct
    ...
    // divides all the values in the heap by 'divisor'. This will affect the values of keys that
    // have been removed.  O(n)
    ...
    // Restores all keys, set their values to zero, and applies a randomised procedure on the keys
    // possibly changing their order  the random_seed controls the randomisation procedure in
    // the following way:      random_seed == -2: the order of the keys initially in the tree
    // structure will be in a fixed increasing order      random_seed == -1: the random_seed
    // will be replaced by the current time, and then randomises the order      random_seed > 0:
    // the randomises the order using the given seen
