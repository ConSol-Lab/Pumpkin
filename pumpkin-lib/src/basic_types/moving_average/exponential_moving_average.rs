...
The idea is to include a smoothing factor to eliminate the strong bias of the first term.

The bias is taken into account as described in the paper, i.e. X_smooth is used instead of X, and X_smooth is gradually increased until it reaches X
...
impl MovingAverageInterface for ExponentialMovingAverage {
    //adds a term to the exponential moving average according to the formula above, including smoothing.
    //The input term is expected to be in a proper integer, e.g., not given in fixed-point arithmetic form.
    fn add_term(&mut self, new_term: u64) {
        ...
        //considering increasing the X_current if its not yet at its target value, this is part of smoothing
        self.x_current = min(self.x_current + 1, 32);
    }

    fn value(&self) -> f64 {
        //recall that the most significant 32 bits hold the integer part, whereas the least significant 32 bits hold the decimal part
