...
pub(crate) fn should_enqueue<Var: IntVar + 'static>(
    ...
    pumpkin_assert_extreme!(
        updated_task.id.unpack() as usize >= parameters.bounds.len() || context.lower_bound(&updated_task.start_variable) > parameters.bounds[updated_task.id.unpack() as usize].0
